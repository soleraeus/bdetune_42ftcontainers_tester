...
# Verifying vector location has been provided
if [ -z ${VECTOR_LOC} ]
then
	echo -e "${RED}Vector location not provided in config.sh${OFF}\n"
	exit 1
elif [ ${VECTOR_LOC::1} == "." ]
then
...
# Verifying vector location is valid
if ! test -f "${VECTOR_LOC}/vector.hpp" ;
then
	echo -e "${RED}vector.hpp not found at ${VECTOR_LOC}/vector.hpp${OFF}\n";
	exit 1;
fi
