...
# Verifying map location has been provided
if [ -z ${MAP_LOC} ]
then
...
# Verifying map location is valid
if ! test -f "${MAP_LOC}/map.hpp" ;
then
	echo -e "${RED}map.hpp not found at ${MAP_LOC}/map.hpp${OFF}\n";
	exit 1;
fi
...
then
	...
	then
		...
		for f in ${testList[@]}; \
		do
			make --no-print-directory SRC="${f}" NAME="${f::-4}" MAP_LOCATION="${MAP_LOC}"
			make --no-print-directory SRC="${f}" NAME="${f::-4}" ISFT=1 MAP_LOCATION="${MAP_LOC}"
		done
		rm -rf logs/*; rm -rf diff/*
	elif [ "$1" == "benchmark" ]
	then
		...
		do
			printf '%-35s   ' $f;
			if ! make --no-print-directory SRC="${f}" NAME="${f::-4}" MAP_LOCATION="${MAP_LOC}" &>/dev/null;
			then
				printf "${FORMAT}" "-" "-" "-"; continue;
			else
				if ! make --no-print-directory SRC="${f}" NAME="${f::-4}" ISFT=1 MAP_LOCATION="${MAP_LOC}" &>/dev/null;
				then
